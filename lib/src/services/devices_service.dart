...
class OlympusDevicesService {
  ...
  Future<Device> enroll(String deviceId, String profile) async {
    final json = await _http.post('/auth/devices/register', data: {
      'device_id': deviceId,
      'profile': profile,
    });
  ...
  Future<void> setKioskMode(String deviceId, String appId) async {
    await _http.post('/platform/device-policies/$deviceId/kiosk', data: {
      'app_id': appId,
      'enabled': true,
    });
  }
  ...
  Future<void> pushUpdate(String deviceGroupId, String version) async {
    await _http.post('/platform/device-policies/updates', data: {
  ...
  Future<List<Device>> listDevices({String? locationId}) async {
    final json = await _http.get('/diagnostics/devices', queryParameters: {
