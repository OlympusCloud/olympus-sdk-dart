...
class OlympusGatingService {
  ...
  Future<bool> isEnabled(String featureKey) async {
    final json = await _http.post('/policies/evaluate', data: {
  ...
  Future<Map<String, PolicyResult>> evaluateBatch(
    ...
    final json = await _http.post('/policies/evaluate/batch', data: {
    ...
    return results.map(
      (key, value) => MapEntry(
        key,
        PolicyResult.fromJson(value as Map<String, dynamic>),
      ),
    );
  }
  ...
  Future<List<Map<String, dynamic>>> listFeatureFlags() async {
    ...
    final items = json['feature_flags'] as List<dynamic>? ??
